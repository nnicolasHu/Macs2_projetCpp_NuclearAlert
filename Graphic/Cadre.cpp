...
void SalleDeControle(sdl2::window* fenêtre, centrale* C){//centrale* C
    ...
    rendement.at(60, 90);
    sdl2::texte pourcentage(std::to_string( arrondi( (C->get_Circuit_Primaire()->get_F_pompe())*100, 0 ) )+"%"s, fonte_texte, *fenêtre, {0xAD, 0xFF, 0x2F, 0});
    pourcentage.at(300, 90);
    sdl2::texte temperature("Température : "s, fonte_texte, *fenêtre, {0xAD, 0xFF, 0x2F, 0});
    temperature.at(60, 110);
    sdl2::texte degre(std::to_string(C->get_Circuit_Primaire()->get_Temperature()) + "°C"s, fonte_texte, *fenêtre, {0xAD, 0xFF, 0x2F, 0});
    degre.at(300, 110);
    sdl2::texte debit("Débit d'eau : "s, fonte_texte, *fenêtre, {0xAD, 0xFF, 0x2F, 0});
    debit.at(60, 130);
    sdl2::texte mcube(std::to_string(C->get_Circuit_Primaire()->get_Debit_eau()) + "m^3/s"s, fonte_texte, *fenêtre, {0xAD, 0xFF, 0x2F, 0});
    mcube.at(280, 130);
    sdl2::texte pression("Pression : "s, fonte_texte, *fenêtre, {0xAD, 0xFF, 0x2F, 0});
    pression.at(60, 150);
    sdl2::texte bar(std::to_string(C->get_Circuit_Primaire()->get_Pression()) + "bar"s, fonte_texte, *fenêtre, {0xAD, 0xFF, 0x2F, 0});
    bar.at(300, 150);
    sdl2::texte radioactivite("Radioactivité : "s, fonte_texte, *fenêtre, {0xAD, 0xFF, 0x2F, 0});
    radioactivite.at(60, 170);
    sdl2::texte bec(std::to_string(C->get_Circuit_Primaire()->get_Radioactivite()) + "Bq"s, fonte_texte, *fenêtre, {0xAD, 0xFF, 0x2F, 0});
    bec.at(300, 170);
    *fenêtre  << pression << bar << radioactivite << bec << debit << mcube << degre<< temperature << titre << rendement << pourcentage << sdl2::rectangle({50, 50}, {340, 150}, {0xAD, 0xFF, 0x2F, 0}, false);
    ...
    sdl2::texte pourcentage2(std::to_string(C->get_Circuit_Secondaire()->get_F_pompe()) + "%"s, fonte_texte, *fenêtre, {0xAD, 0xFF, 0x2F, 0});
    pourcentage2.at(300, 260);
    sdl2::texte temperature2("Température : "s, fonte_texte, *fenêtre, {0xAD, 0xFF, 0x2F, 0});
    temperature2.at(60, 280);
    sdl2::texte degre2(std::to_string(C->get_Circuit_Secondaire()->get_Temperature()) + "°C"s, fonte_texte, *fenêtre, {0xAD, 0xFF, 0x2F, 0});
    degre2.at(300, 280);
    sdl2::texte debit2("Débit d'eau : "s, fonte_texte, *fenêtre, {0xAD, 0xFF, 0x2F, 0});
    debit2.at(60, 300);
    sdl2::texte mcube2(std::to_string(C->get_Circuit_Secondaire()->get_Debit_eau()) + "m^3/s"s, fonte_texte, *fenêtre, {0xAD, 0xFF, 0x2F, 0});
    mcube2.at(280, 300);
    sdl2::texte pression2("Pression : "s, fonte_texte, *fenêtre, {0xAD, 0xFF, 0x2F, 0});
    pression2.at(60, 320);
    sdl2::texte bar2(std::to_string(C->get_Circuit_Secondaire()->get_Pression()) + "bar"s, fonte_texte, *fenêtre, {0xAD, 0xFF, 0x2F, 0});
    bar2.at(300, 320);
    sdl2::texte radioactivite2("Radioactivité : "s, fonte_texte, *fenêtre, {0xAD, 0xFF, 0x2F, 0});
    radioactivite2.at(60, 340);
    sdl2::texte bec2(std::to_string(C->get_Circuit_Secondaire()->get_Radioactivite()) + "bq"s, fonte_texte, *fenêtre, {0xAD, 0xFF, 0x2F, 0});
    bec2.at(300, 340);
    *fenêtre << rendement2 << pourcentage2 << temperature2 << degre2 << debit2 << mcube2 << pression2 << bar2 << radioactivite2 << bec2<< circuit_secondaire << sdl2::rectangle({50, 220}, {340, 150}, {0xAD, 0xFF, 0x2F, 0}, false);
    ...
    sdl2::texte MW(std::to_string(C->get_MW()) + "MW"s, fonte_texte, *fenêtre, {0xAD, 0xFF, 0x2F, 0});
    MW.at(300, 440);
    *fenêtre << production << MW << sdl2::rectangle({50, 390}, {340, 100}, {0xAD, 0xFF, 0x2F, 0}, false);
    ...
    sdl2::texte bar3(std::to_string(C->get_P_enceinte()) + "bar"s, fonte_texte, *fenêtre, {0xAD, 0xFF, 0x2F, 0});
    bar3.at(300, 560);
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    //////////////////////////////////////////////////////////////////////////////
    // A vérifier
    sdl2::texte pourcentage3(std::to_string(C->get_Circuit_Secondaire()->get_F_condenseur()) + "%"s, fonte_texte, *fenêtre, {0xAD, 0xFF, 0x2F, 0});
    ...
    //////////////////////////////////////////////////////////////////////////////
    // A vérifier
    sdl2::texte mcube3(std::to_string(C->get_Circuit_Secondaire()->get_D_condenseur()) + "m^3/s"s, fonte_texte, *fenêtre, {0xAD, 0xFF, 0x2F, 0});
    mcube3.at(710, 130);
    //////////////////////////////////////////////////////////////////////////////

    sdl2::texte differ_temp("Différence de température : "s, fonte_texte, *fenêtre,{0xAD, 0xFF, 0x2F, 0});
    differ_temp.at(430, 160);
    sdl2::texte diff_temperature(std::to_string(C->get_Circuit_Secondaire()->get_Delta_ES()) + "°C"s, fonte_texte, *fenêtre, {0xAD, 0xFF, 0x2F, 0});
    ...
    *fenêtre << Rend_Cond << sys_froid << pourcentage3 << debit_cond << mcube3 << differ_temp << diff_temperature << sdl2::rectangle({420, 50}, {400, 150}, {0xAD, 0xFF, 0x2F, 0}, false);
    ...
    sdl2::texte pourcentage4(std::to_string(C->get_Reacteur()->get_E_barre()) + "%"s,fonte_texte, *fenêtre, {0xAD, 0xFF, 0x2F, 0});
    ...
    *fenêtre << barre_graph << pourcentage4 << sdl2::rectangle({420, 220}, {400, 100}, {0xAD, 0xFF, 0x2F, 0}, false);
    ...
    sdl2::texte pourcentage5(std::to_string(C->get_Reacteur()->get_TBore_eff()) + "%"s,fonte_texte, *fenêtre, {0xAD, 0xFF, 0x2F, 0});
